...
func largeDocumentEditPerformance() {
    ...
    #if DEBUG
    if !TestEnvironment.isCI { #expect(mean < 15.0 * TestEnvironment.perfBudgetScale) }
    if !TestEnvironment.isCI { #expect(median < 15.0 * TestEnvironment.perfBudgetScale) }
    if !TestEnvironment.isCI { #expect(p95 < 30.0 * TestEnvironment.perfBudgetScale) }
    #else
    if !TestEnvironment.isCI { #expect(mean < 5.0 * TestEnvironment.perfBudgetScale) }
    #if DEBUG
    if !TestEnvironment.isCI { #expect(median < 15.0 * TestEnvironment.perfBudgetScale) }
    if !TestEnvironment.isCI { #expect(p95 < 30.0 * TestEnvironment.perfBudgetScale) }
    #else
    if !TestEnvironment.isCI { #expect(median < 5.0 * TestEnvironment.perfBudgetScale) }
    if !TestEnvironment.isCI { #expect(p95 < 10.0 * TestEnvironment.perfBudgetScale) }
    #endif
    #endif
