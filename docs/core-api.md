...
**`_assets/` is reserved** (FR-2.5, deferred — ADR-040). `<root>/_assets/` is
...
`FilawayApp/Features/Editor/PasteIntelligence*.swift` (ADR-038).
...
Import → Apple Notes…** menu item so the sentence exists once. See ADR-039.
