...
## ADR-037 — Onboarding is a modal window, and the first read of the notes root runs it
...
## ADR-038 — Paste intelligence offers; it never rewrites the paste
...
## ADR-039 — Deferred features ship as a contract plus a disabled menu item
...
## ADR-040 — `_assets/` is reserved now so attachments can arrive later
...
## ADR-041 — FR-4.7 "Reorganize library" is cut from Phase 1
