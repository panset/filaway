...
## ADR-012 — AI usage lives in its own SQLite file, not in `filaway.sqlite`
...
## ADR-013 — Versioned prompts are `FilawayCore` resources, not a top-level `Prompts/`
...
## ADR-014 — Replay fixtures are keyed by a canonical request hash and store wire bodies
...
## ADR-015 — Merge is `moveSegment`, and the plan carries the segment text verbatim
...
## ADR-016 — The plan tool schema is a discriminated `anyOf`
