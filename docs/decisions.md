...
## ADR-022 — The bge-small `.mlpackage` is committed as a `FilawayCore` resource
...
## ADR-023 — Vectors are binary16 bytes, converted with vImage, not Swift `Float16`
...
## ADR-025 — Hybrid ranking is RRF, and a date range filters before the top-k cut
