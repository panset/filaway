...
## ADR-041 — Sparkle comes from SPM, and it brings its own command-line tools
...
## ADR-042 — "Updates not configured" is a real implementation, not an error path
...
## ADR-043 — Not sandboxed, so Sparkle's XPC services are stripped
...
## ADR-044 — CFBundleVersion is the commit count, and the appcast is signed per tag
