...
## ADR-054 — The answer card has a local arm, and the Claude arm is raced against a clock
...
## ADR-055 — Prompt chunks are numbered, and a reported snippet must be verbatim
...
## ADR-056 — ⏎ is the semantic trigger, and the answer card is the first selectable item
...
## ADR-057 — An unreadable database is quarantined, not deleted; diagnostics carry no path under the notes root
...
## ADR-058 — Changing the notes folder *opens* a library; it never moves one
...
## ADR-059 — One status pill, fed by the organizer, with the connection folded in
...
## ADR-060 — The accessibility check walks the tree, and audits itself first
