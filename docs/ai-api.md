...
Its own database file, not a `MetadataStore` migration — see ADR-012. Replayed
...
`moveSegment` is plan §1 amendment 1's "merge = move segment" (ADR-015): the
...
The schema is a discriminated `anyOf` of seven closed objects (ADR-016 — the one
...
SwiftPM resources of `FilawayCore` (ADR-013 — this is the package's only
...
1. That the API accepts the `anyOf` schema under `strict: true` (ADR-016).
